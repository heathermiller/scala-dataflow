...
\begin{frame}
  ...
  \begin{columns}[t]
    ...
    FlowSeq: Barrier-Free, immutable ParArray
    %% TODO better thing?
...
\begin{frame}
  ...
  \framesubtitle{FlowArrays / FAJobs}

  \begin{columns}[t]
    \column{.5\textwidth}
    \begin{block}{FlowArray}
      \begin{itemize}
      \item Exposed to user
      \item Hold actual data
      \item Generated by FAJobs
      \end{itemize}
    \end{block}
...
\begin{frame}
  ...
  \begin{alltt} \small
    \sK{val} \sV x = FlowArray.tabulate(size)(x => x*x)\\
    \sK{val} \sV y = FlowArray.tabulate(size)(x => x*x)\\
    \sS
...
\begin{frame}
  ...
  %% TODO remove ParArray in normalized time graph
...
\begin{frame}
  ...
  \pause
  \begin{alltt}
    \sK{val} \sV{fa1} = x.flatMapN(n*n)(x => x.flatMapN(n)( ... ))\\
    \sK{val} \sV{fa2} = x.flatMapN(n)(...).flatMapN(n)(...)
  \end{alltt}
\end{frame}
