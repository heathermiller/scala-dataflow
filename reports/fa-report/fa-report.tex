...
\begin{abstract}
  ...
  parallel datastructure, the FlowPools. Attempts to implement a
  bigger application using FlowPools, showed that the lack of ordering
  is very limiting. FlowArrays, proposed by this report, do not have
  ...
  ParArrays. Further, we will find that with the current storage model
  of FlowArrays, garbage collection takes considerably more time than
...
First we give an overview of the general idea behind FlowArrays and
how exactly they try to improve on ParArrays. The section outlines the
basic concepts of how task splitting in FlowArrays is done. The next
section describes how FlowArrays are implemented in detail. The
...
\item The design and Scala~\cite{Odersky10} implementation of a
  ParArray-like, but non-blocking datastructure.
...
In this section we will first outline how FlowArrays strive to improve
on ParArrays from an abstract perspective. Then we will show some of
the difficulties that arise particularly with this approach.

\subsection{Barrier-Freedom}
FlowArrays are, unlike ParArrays, barrier-free: We will use a simple
...
operation, which may be another FlowArray as in the case of
...
inner FlowArrays is big (otherwise something different than a FlowArray
...
would (almost)
always be the root job, and hence splitted dependency tracking would
...
Another idea is to make the FlowArray structure inherently aware of
...
We have tested the performance of FlowArrays against
...
GC cylces. No full GC cycles have been performed during the
...
flow based programming model: a ParArray (or any other imperative,
