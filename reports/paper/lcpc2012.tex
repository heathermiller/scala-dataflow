...
different edges of the DAG independently of each other.
...
state can be manipulated by several threads, using streams or iterators can
result in nondeterministic execution.
...
an approach to \verb=seal=ing which simply prevents calling append
from the point when "seal" is called, however, yields a
...
The semantics of \verb=seal= is that it fails if the pool
is already sealed with a different size or the number of elements
is greater than the desired size. Note that it is not guaranteed that the
same exception will occur on the same thread-- rather, we guarantee 
that if \emph{any} exception is thrown by \emph{any} thread, any
execution schedule will result in \emph{some} exception.
...
for additional examples), which recursively applies \verb=f= to each number and
...
This corresponds to chaining or pipelining the nodes in a dataflow graph.
...
Each FlowPool can also potentially be in a sealed state, meaning there is a bound on
...
See appendix~\ref{sec:add-eval} for further evaluation.
...
most occasions, meanwhile also avoiding CAS failures. This appears to reduce execution
...
In future work, we plan on developing other
...
bounded buffers, dataflow streams and maps.
