...
%   Each lane of a MLFP is a tagglable abstract pool. (No proof given).
% \end{lemma}

% \begin{definition}[MultiLaneFlowPool]
%   A MLFP $mpool$ is an object that has a reference to multiple taggable
%   abstract pools $mpool.lanes$ and a global seal state
%   $mpool.sstate$. The MLFP state is defined as the states of all the
%   lanes and the global seal state.
% \end{definition}

% \begin{definition}[Consistency]
%   A multi-lane FlowPool state $\mathbb{S}$ with is \textbf{consistent} with
%   an abstract pool $\mathbb{P} = (elems, callbacks, seal)$ at $t_0$ if
%   and only if $\mathbb{S}$ is a valid state and:
%   \begin{description}
%   \item $\forall e \in Elem : \exists l \in lanes : hasElem(l.start,e)
%     \Leftrightarrow e \in elems$
%   \item $\forall f \in Elem => Unit : \forall l \in lanes :
%     hasCallback(l.start,l) \Leftrightarrow f \in callbacks$
%   \item $\forall f \in Elem => Unit : \forall e \in Elem : \forall l \in
%     lanes : willBeCalled(l.start, e, f) \Leftrightarrow \exists t_1 \ge
%     t_0 : \forall t_2 > t_1 : \mathbb{P}(t_2) = (elems_2,(f,called_2)
%     \cup callbacks_2, seal_2), elems \subset called_2$
%   \item $\forall s \in \mathbb{N}_0 \forall l \in lanes \exists v \in $
%   \end{description}
...
%     cbs = \bigcap_{i=1}^N  cbs_i \label{eq:mlfp-aggreg}
%   \end{equation}
% \end{theorem}

% \begin{proof}
%   Given a MLFP in its initial state $(\emptyset, \emptyset, -1)$ and
%   hence with all lanes in state $(\emptyset, \emptyset, -1)$,
%   trivially, we have \eqref{eq:mlfp-aggreg}.

%   For \verb+append+ we have
% \end{proof}

% \begin{theorem}[Linearizability]
%   MLFP operations \verb+append+ and \verb+seal+ are linearizable. (No
%   proof given).
% \end{theorem}
