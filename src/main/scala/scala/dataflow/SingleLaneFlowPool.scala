...
class SingleLaneFlowPool[T] extends FlowPool[T] {
  ...
  val initBlock = SingleLaneFlowPool.newBlock(0)
  
  def newPool[S] = new SingleLaneFlowPool[S]()

  def builder: SingleLaneBuilder[T] = new SingleLaneBuilder[T](initBlock)
  ...
  private def seal(size: Int, curblock: Array[AnyRef], pos: Int) {
    curblock(pos) match {
      ...
      case cbl: CallbackList[_] =>
        if (pos < SingleLaneFlowPool.LAST_CALLBACK_POS) {
  ...
  def awakeCallback() {
    ...
    if (lk < 0) {
      ...
      if (tryOwn()) {
        ...
        SingleLaneFlowPool.task(new CallbackElem.BatchTask(this))
      } else awakeCallback()
    }
  }
      ...
      if (callback.pos >= LAST_CALLBACK_POS) {
        block(MUST_EXPAND_POS) match {
          ...
          case Next(b) => if (callback.tryOwn()) {
            callback.block = b
            callback.pos = 0
            SingleLaneFlowPool.task(new CallbackElem.BatchTask(callback))
          }
        }
      } else {
        cur = block(pos)
