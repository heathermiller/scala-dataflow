...
dependency graph in which parallelism is achieved by operations in the
...
in an imperative model with a side-effecting accumulator.}
...
Book by \cite{RoyH2004} describes the Oz programming language 
a subset of which yields programs deterministic by construction.
The dataflow streams described there are built on top of
single-assignment variables and are deterministically ordered.
While they allow multiple consumers, only one producer can write
at a time.
Oz has its own runtime, thus assuming continuation-based blocking
can be implemented efficiently, which is not the case on the JVM,
CLR and many native environments.
...
interesting to mention, because they define monadic operators and a
number of high-level combinators which produce new futures.

The concept of single assignment variables has been 


Things to probably cite: Oz, gpars, i-Structures \cite{Arvind89}, FlumeJava \cite{Chambers10}.
...
Java, FlumeJava...
	...
	- Nir Shavit, Data Structures in the Multicore Age
        - type-systematic approach to determinism - cite Deterministic
        Parallel Java and X10's dependent types
        - need to go through the work of the program committee
...
futures and FlowPools -- this offers a particular benefit on platforms
which do not by default support efficient continuations.
