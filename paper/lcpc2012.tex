...
\textbf{INV1} let $b: Block$ such that $reachable(start,b)\wedge b.next=\bot$. 
...
$\mathbb{A} \in \{(elems,sealed,callbacks)~|~elems \subset Elem,~sealed \in \{-1\} \cup \mathbb{N},~callbacks\subset Elem \Rightarrow Unit\}$ 
...
$seal(\mathbb{A},sealSize) = \mathbb{A''}$ where $\mathbb{A''} = (elems, sealSize, callbacks)~:~sealSize \in \mathbb{N}$ 
...
non-consistency changing instruction after a finite number of steps.
...
$term.sealed\in\mathbb{N}$, or in the case of a $doForAll$ invocation, 
$\{term.callbacks\in$\}). 
\end{itemize}
...
Finally, if $advance$ finds that the element at $idx$ is an $Elem$, it will
increment $idx$. By $INV1$, this can only happen a finite number of times
until a $Terminal$ is found. In the case that $expand$ is meanwhile invoked
through $advance$, by Lemma 2 it's guaranteed to complete state changing
instructions $CAS3$ or $CAS4$ in a finite number of steps. Otherwise, some
other thread has moved the $Terminal$ to a subsequent index. However, this
latter case is only possible by successfully completing $CAS2$, a consistency
changing instruction, after the initial invocation of append.
...
\item \textbf{Case 1:} The object at the current index location $idx$ is of
type $Elem$, not $Terminal$, causing a recursive call to $seal$ with the same
size $size$. 

% proof about the WRITE in Elem case. Assuming one can reliably write the index...

% then we can show that idx will eventually come to a terminal. DONE via INV1.
% therefore, we will be able to complete seal.
...
% have to prove here that after a finite # of steps, we end up in case 1.

% we also have to fix the current pointer in the case where it's broken,
% necessitating a call to expand. We know by the expand lemma that the current
% pointer will be updated after a finite number of steps to point to a
% subsequent block.

% if after expand but before READ(current) current.index > LASTELEMPOS, then a
% consistency change must've occurred because append must've been completed.
% otherwise, we'd go to case 1.

% then we have to somehow fix the expand lemma

% CRUFT
% =====
% Here, we have to show that idx = LASTELEMPOS after a finite number of
% recursive calls.
% In this case, we show that the check $idx < LASTELEMPOS$ will evaluate to
% $true$ in some finite number of steps. 
% After a finite number of steps, we'll end up in Case 2.

% either the initial invocation of seal completes, by lemma 6, or a
% consistency changing operation completes.

% Here, we have to show that we don't infinitely invoke expand 
% We know by the expand lemma that the current pointer will be updated after a
% finite number of steps to point to a subsequent block.

% expand is possibly incorrect because the lemma claims that when you invoke
% expand, you're guaranteed to create a block and numBlocks monotonically
% increases
...
\textbf{Lemma 9}. \textit{Assume a concurrent operation $op$ is started. After
...
\textbf{Lemma 10}. \textit{Assume a concurrent operation is started. If a
...
By Lemma 2, we know that when we call
$expand$, it's guaranteed that there will be a finite number of steps executed
followed by a state changing operation.


After invoking append, if check is false, a consistency changing CAS (CAS2, CAS5, or CAS6) is completed after a finite number of steps.
