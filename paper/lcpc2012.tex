...
finite number of execution steps, as they contains no recursive calls, loops,
or other possibility to restart.

The \textbf{$append$} operation is restarted in one of three cases:
\begin{itemize} 
\item \textbf{Case 1:} the call to the finite operation $check$ fails, 
indicating that either at the current index location $idx$ in the block $b$ 
is not a $Terminal$ or that $idx > LASTELEMPOS$, requiring in invocatin of 
$advance$, which was shown above to complete in a finite number of steps, 
followed by a recursive call $append(elem)$, on the same element $elem$. (still have to show that Case 1 is bounded somehow)
\item \textbf{Case 2:} $CAS1$ fails, which we know from Lemma 1 means that it 
must've already been completed by another thread, triggering a recursive call 
$append(elem)$ again on the same element $elem$.
\item \textbf{Case 3:} $CAS2$ fails, indicating that after the (have to prove here that after the read of currobj, some thread must have succeeded)
\end{itemize}
...
If the check fails, "advance" is called. If the index read at the beginning of "advance" is not greater than LASTELEMPOS, the index is updated to index + 1 using WRITE2. After that, append is repeated.

We show that after a finite number of steps, the check will evaluate to true, or some other thread will have completed a consistency changing operation since the initial invocation of append. In the case where check evaluates to true, the previous Lemma applies ("When check is true it is guaranteed that a consistency changing CAS is completed after a finite number of steps").

First, if advance finds that the element at the current index is a Terminal, then the next invocation of append will evaluate the check to true, or it must be the case that another thread has moved the Terminal to a subsequent index since the initial invocation of append, which is only possible using a consistency changing instruction.

Second, as long as "advance" finds that the element at the current index is an Elem, it will increment the index. This can only happen a finite number of times until a Terminal is found in the current block, or some other thread has moved the Terminal to a subsequent index. However, the latter is only possible by successfully completing CAS2 after the initial invocation of append.
