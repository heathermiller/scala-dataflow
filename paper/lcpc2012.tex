...
$\exists i$ such that $b.array(i) = Terminal \wedge \forall j>i$ 
such that $j\leq LASTELEMPOS, b.array(j) = \bot$
...
strictly increases after some finite number of steps.  \qed
...
%==============================
% LEMMA 4 (FINITE EXEC BETWEEN CAS-ES)
%==============================
\textbf{Lemma 4}. \textit{Each operation executes only a finite number of steps
between each state changing instruction}.

\textit{Proof}. We proceed by individually inspecting each operation which
contains a state changing instruction.

The \textbf{advance}, \textbf{check}, \textbf{totalElems},
\textbf{invokeCallbacks}, and \textbf{tryWriteSeal}  operations have a
finite number of execution steps, as they contain no recursive calls, loops,
or other possibility to restart.

The \textbf{append} operation is restarted in one of three cases:
\begin{itemize} 

% This case still needs a bit of work, and an additional invariant or two
\item \textbf{Case 1:} The call to the finite operation $check$ returns $false$, 
causing call to $advance$ followed by a recursive call to $append$ with the same 
element $elem$ which in turn once again calls $check$. 
...
% Mention something about the expand lemma here since expand is being invoked?
% Do we need to give some other proof that this is terminating, i.e. that there
% are certainly a finite number of elements before a Terminal block?
% Add an invariant on initialization so as to address the fact that there is 
% always at least one, but at most two Terminal objects in pool.array?
...
must've already been completed by another thread, a state changing instruction.
...
\textbf{Lemma 5}. \textit{Assume a concurrent operation $op$ is started. After
...
\textbf{Lemma 6}. \textit{Assume a concurrent operation is started. If a
