...
Second, as long as $advance$ finds that the element at $idx$ is an $Elem$, it
will increment $idx$. This can only happen a finite number of times until a
$Terminal$ is found in the current block, or some other thread has moved the
$Terminal$ to a subsequent index. However, the latter is only possible by
successfully completing $CAS2$ after the initial invocation of append.
