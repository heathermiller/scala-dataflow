...
\begin{lemma}[Substring]\label{lemma-substring}
...
\rightarrow cb_m'$ is a substring of $null \rightarrow cb_1 \rightarrow \cdots \rightarrow cb_n$.
\end{lemma}
...
We have to prove that it is actually a substring.
...
changed back to $cb_0$ as we know from the lemma \ref{lemma-freshness}.
\end{proof}
...
\ref{lemma-substring}. Due to the same lemma, the entries of
...
It remains to shown that $willBeCalled(start, e, f)$ holds at $t_3$.
