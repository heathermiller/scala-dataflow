...
Plus, iterators or streams would not be deterministic. Even if they
are thread-safe, they have an inherent state which can lead to nondeterminism.

what should be the semantics of the foreach - should it execute only
for elements added to the flowpool after calling foreach? or should it
execute for all the elements ever added?
The latter -- we want determinism, avoid races, same execution every
time.

But this leads to GC problems! We have to keep every element around
forever.
Computations can run arbitrarily long (GC point)
...
What about the `reduce` patterns? How can you get back reduction of
all the values in the flowpool if at any point an additional value
could potentially arrive.
Plus, at some point we want to release callbacks as well -- we do not
want to keep them around forever.

This is why we have the `seal` operation.

But, calling seal can lead to nondeterministic computations too!
To avoid nondeterminism, there has to be some sort of on agreement on
the state of the pool when it is sealed.

One convenient and sufficient information to make `seal` deterministic
is the expected size of the pool.

Show how this can be used to implement reducing (we assume existence
of promises in the framework).
...
least one operation completes after a finite number of steps.
