...
\urldef{\mailsa}\path|firstname.lastname@{epfl.ch, typesafe.com}|
...
\title{FlowPool: A Lock-Free Deterministic Concurrent Dataflow Abstraction}

% a short form should be given in case it is too long for the running head
\titlerunning{FlowPool: A Lock-Free Deterministic Concurrent Dataflow Abstraction}
...
\authorrunning{FlowPool: A Lock-Free Deterministic Concurrent Dataflow Abstraction}
...
\begin{enumerate}
\item The design and Scala implementation\footnote{See \texttt{http://www.assembla.com/code/scala-dataflow/git/nodes}}
...
in appendix \ref{appendix-correctness}.
...
proof. A complete formal proof can be found in appendix 
...
\ref{fig:eval-cpu-scaling} \& \ref{fig:eval-hist-comm}. Further evaluation can
be found in appendix~\ref{sec:add-eval}.
...
In the \emph{Insert} benchmark, Figure \ref{fig:eval-cpu-scaling}, we evaluate the ability to write
concurrently, by distributing the
...
to 400 million elements, indicating that unneeded elements are being properly garbage collected.
...
deterministic. And finally, experimental results show that in most practical
cases, FlowPools considerably outperform comparable concurrent queue data
structures present in the Java standard library.
...
\textbf{Note to the reviewers.} For reference, we provide an
appendix which contains (a) proofs of the presented lemmas and
theorems, (b) a summary and examples of the syntax used in pseudocode,
and (c) additional evaluation results. The appendix is not part of the
paper, but is provided for convenience. It is planned to form part of
