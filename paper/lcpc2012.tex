...
than programs based on mainstream concurrency constructs, such as shared-
memory threads and locks, as programmers are only required to specify data and
...
% I don't like how it says "we go on to show"
Finally, we show that FlowPools are able to overcome practical pitfalls which
are not uncommon to functional programming models, such as out-of-memory
errors, enabling programs based upon FlowPools to run indefinitely. This is
made possible by implementing FlowPools without the use of iterators or
streams. By using a \textit{builder} abstraction internally, we are able to
not only avoid the non-determinism caused by iterators and streams, but we are
additionally able to garbage collect parts of the data structure we no longer
need, thus reducing memory consumption, and improving cache behavior due to
fewer GC cycles.
