...
TODO What is producer-consumer parallelism.

TODO Description and image of queue/stream of values, producer, and multiple consumers.
A directed dependency graph of a dataflow computation provides parallelism.
This ensures that we have some parallelism in the model.

TODO Nodes in dependency graph should be first class model.
...
programming language \ref{TODO} -- a concurrent program is declaratively concurrent
...
We assume that the \verb=foreach= returns value \verb=Future[Int]=
...
\begin{lemma}[CAS2]\label{lemma-cas2} After invoking \verb=append(elem)=, if
