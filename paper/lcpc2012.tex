...
These allow multiple consumers, but only one producer writes
at a time.
...
Futures have been generalized to data-driven tasks and futures,
which provide additional information for the
scheduler \cite{Tasirlar11}.
...
FlumeJava \cite{Chambers10} relies heavily on the concept of
collections containing a set of futures.
The CnC model \cite{Burke11} is a parallel programming model
...
X10's constrained-based dependent-value types can similarly ensure
determinism and deadlock-freedom \cite{Saraswat07}.


Things to probably cite: Oz, gpars.
