...
capabilities, FlowPools may reduce execution time by $53 - 97\%$ in
highly parallelized environments with respect to comparable
concurrent queue data structures in the Java standard library.
...
values. This makes it possible to use FlowPools as arguments or to receive
...
be specified at compile time, but can be generated dynamically at runtime
instead.
...
resulting data structures. For a more formal definition, see section
\ref{sec:correctness}.
...
most occasions, while avoiding CAS failures. Execution time decrease
up to $98\%$ for $5000000$ elements on 32-core Xeon, $P = 8$.

Usage of the inserted data is evaluated in the \emph{Reduce},
\emph{Map} and \emph{Histogram} benchmark, where the latter --
serving as a ``real live'' example here -- uses the former two
...
$P$, for multi-lane FlowPools). %% TODO the parenthesis is bad
...
longer needed. Execution time decrease up to $95\%$ for $1000000$ on
32-core Xeon, $P = 16$ (for $P \ge 8$).
...
\caption{CPU scaling properties of benchmarks.
  $\Diamond$ SingleLaneFlowPool,
  $\times$ MultiLaneFlowPool,
  $\triangle$ LinkedTransferQueue,
  $\circ$ ConcurrentLinkedQueue} \label{fig:eval-cpu-scaling}
\end{figure}
...
reduction of a term $t$, such that reduction steps are applied in the
canonical order.
\end{definition}
...
refer to FlowPools from the \textsc{Create} steps we have moved to the
beginning of the interleaving.
...
created the same set of threads as in the original interleaving.
...
once the pool is sealed. Execution time decrease by up to $95\%$ for
$10000000$ elements on 32-core Xeon, $P = 8$.

The \textit{Map} benchmark is similar to the \textit{Reduce}
benchmark, but instead of reducing a value, each element is mapped
into a new one and added to a second pool. Execution time decrease by
up to $96\%$ for $10000000$ elements on 32-core Xeon, $P = 8$.
...
$53-97\%$. These figures have been obtained by comparing medians of
execution times for each configuration with $P = 8$ (the highest level
of parallelism we have for all architectures), both for multi-lane
FlowPools and the faster of linked transfer queues and concurrent
linked queues. The higher bound results from the insert benchmark on
32-core Xeon with $5000000$ insertions. The lower bound results from
the histogram benchmark on UltraSPARC T2 with $500000$ elements.
...
%% TODO
% chache line transfer collision argument
...
\caption{Size scaling properties of benchmarks ($P = 1,
  8$). $\Diamond$ SingleLaneFlowPool,
  $\times$ MultiLaneFlowPool,
  $\triangle$ LinkedTransferQueue,
  $\circ$ ConcurrentLinkedQueue} \label{fig:eval-size-scaling}
\end{figure}

\begin{figure}
\includegraphics[width=\textwidth]{../benchmarks/graphs/lanef-scaling}
\caption{Lane-Factor scaling properties of Multi-Lane FlowPool} \label{fig:eval-lane-factor}
\end{figure}
