...
\textbf{state} $\mathbb{S}$ is defined as the sequence of blocks reachable from
$pool.start$ by following $next$ references within blocks. More formally, the
relation $reachable(b,c)$ on two blocks $b,$ $c$ holds iff
...
\textbf{Definition 5} (Lock-freedom). In a situation where some finite number
...
\textbf{Lemma 2}. \textit{Assume a concurrent operation $op$ is started. After
a finite number of non-consistency changing CAS instructions, a consistency
changing CAS instruction is guaranteed to be successfully completed}.
