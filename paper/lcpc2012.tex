...
UltraSPARC T2 with 64 hardware threads. On the first architecture we
have $P \in [1, 8]$, on the latter two $P \in [1,64]$. Here, we
discuss the scaling of the said data structures (see
fig~\ref{fig:eval-cpu-scaling}). For additional evaluations, please
refer to section~\ref{sec:add-eval} in the appendix.
...
work of inserting a $N$ elements into the data structure concurrently
across $P$ threads.
...
most occasions, while avoiding CAS failures.
