...
The \textbf{FlowPool state} is defined as the directed graph of objects transitively reachable from the reference $start$.
...
\begin{proof}
Since none of the relations $hasElem$, $willBeCalled$ and $sealedAt$
...
the last non-null element in $b.array$, it does not change the
$sealedAt$ relation either.
Finally, the CAS does not change the set of scheduled futures, so
it does not affect the $willBeCalled$ relation.

The CAS in line \ref{cas_propagate} does not make some new element reachable,
hence the $hasElem$ relation is preserved.

Note now that this CAS does not change the relations $willBeCalled$
...
The argument above proves directly that relations $willBeCalled$
...
relation $hasElem(start, e)$ holds, $sealedAt(start, s)$ did not
change between $t_2$ and $t_3$.
...
\begin{proof}[Safety]
From lemmas \ref{lemma-append}, \ref{lemma-foreach} and
